...
async function run() {
    ...
    await redis.del(`sentinal:gpt-4-turbo:${subjectId}:minute`)
    await redis.del(`sentinal:gpt-4-turbo:${subjectId}:daily_budget`)

    console.log('--- Test 1: Reserve 50 tokens (Limit 100) ---')
    const res1 = await reserveBudget(subjectId, 'gpt-4-turbo', 50, MINUTE_LIMIT, 50, DAILY_LIMIT_CENTS)
    ...
    await adjustBudget(subjectId, 'gpt-4-turbo', 10, 10)
    await new Promise(r => setTimeout(r, 100)) // give redis a moment (pipeline)
    const budget2 = await getRemainingBudget(subjectId, 'gpt-4-turbo')
    ...
    const res3 = await reserveBudget(subjectId, 'gpt-4-turbo', 70, MINUTE_LIMIT, 70, DAILY_LIMIT_CENTS)
    console.log('Res3:', res3)
    if (res3.allowed) throw new Error('Test 3 failed (should be denied)')

    console.log('--- Test 4: Reserve 60 tokens (Exact remaining) ---')
    const res4 = await reserveBudget(subjectId, 'gpt-4-turbo', 60, MINUTE_LIMIT, 60, DAILY_LIMIT_CENTS)
